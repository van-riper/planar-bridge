...
def is_retryable_status(status_code: int) -> bool:
    ...
    Args:
        status_code: The HTTP status code of a failed response.

    Returns:
        True for transient rate-limit and server faults, False for
...
class RetryPolicy:
    ...
    def backoff_for(self, attempt: int) -> float:
        ...
        Args:
            attempt: Zero-based index of the failed attempt.
...
class AsyncHttpClient:  # pylint: disable=too-few-public-methods
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            client: The underlying httpx session. Its lifecycle is owned
                by the caller.
            limiter: The shared limiter every request waits on.
            policy: The retry knobs governing backoff and the attempt
                budget.
            sleeper: Awaitable sleep. Injected for deterministic testing.
        """
        self._client = client
    ...
    async def get(self, url: str) -> httpx.Response | None:
        ...
        Returns:
            The successful response, or None when the request fails
            fatally or exhausts its retries.
        """
